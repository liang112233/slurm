...
static int arg_set_ofname(slurm_opt_t *opt, const char *arg)
{
	...
	xfree(opt->sbatch_opt->ofname);
	if (!xstrcasecmp(arg, "none"))
		opt->sbatch_opt->ofname = xstrdup("/dev/null");
	else
		opt->sbatch_opt->ofname = xstrdup(arg);

	return SLURM_SUCCESS;
}
COMMON_SBATCH_STRING_OPTION_GET(ofname);
COMMON_SBATCH_STRING_OPTION_RESET(ofname);
...
static char *arg_get_wait(slurm_opt_t *opt)
{
	...
	return xstrdup(opt->sbatch_opt->wait ? "true" : "false");
}
